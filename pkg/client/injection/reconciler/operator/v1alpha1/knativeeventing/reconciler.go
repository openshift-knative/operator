...
// controller reconciling v1alpha1.KnativeEventing.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.KnativeEventing. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha1.KnativeEventing) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.KnativeEventing.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.KnativeEventing. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.KnativeEventing) reconciler.Event
}

// reconcilerImpl implements controller.Reconciler for v1alpha1.KnativeEventing resources.
type reconcilerImpl struct {
	// Client is used to write back status updates.
	Client versioned.Interface

	// Listers index properties about resources
	Lister operatorv1alpha1.KnativeEventingLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister operatorv1alpha1.KnativeEventingLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.KnativeEventings(namespace)
...
func (r *reconcilerImpl) updateStatus(existing *v1alpha1.KnativeEventing, desired *v1alpha1.KnativeEventing) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.OperatorV1alpha1().KnativeEventings(desired.Namespace)
		...
		updater := r.Client.OperatorV1alpha1().KnativeEventings(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.KnativeEventing) (*v1alpha1.KnativeEventing, error) {
	finalizerName := defaultFinalizerName

	getter := r.Lister.KnativeEventings(resource.Namespace)
	...
	patcher := r.Client.OperatorV1alpha1().KnativeEventings(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.KnativeEventing) (*v1alpha1.KnativeEventing, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.KnativeEventing, reconcileEvent reconciler.Event) (*v1alpha1.KnativeEventing, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
